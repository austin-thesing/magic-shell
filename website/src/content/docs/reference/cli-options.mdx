...
Magic Shell installs four commands:
- `magic-shell` - Full name (CLI mode)
- `msh` - Short alias (CLI mode)
- `ms` - Shortest alias (CLI mode)
- `mshell` - TUI mode (launches interactive interface directly)

The first three are identical CLI commands. Use `mshell` to launch the TUI directly without flags.
